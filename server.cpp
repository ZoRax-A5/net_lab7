...
void removeClient(sockaddr_in client_addr);
...
void Server::start()
{
    ...
    */
    ...
    while (true)
    {
        ...
        HANDLE hThread = CreateThread(NULL, 0, connection, &client_socket, 0, &thread);
        if (hThread == NULL)
        {
            closesocket(server_socket);
            WSACleanup();
        ...
        else
        {
            ...
            case EXIT:
                ...
                for (auto iter = clients_queue.begin(); iter != clients_queue.end(); ++iter)
                {
                    ...
                    {
                        del_caddr = inet_ntoa(iter->client_addr.sin_addr);
                        del_port = iter->client_addr.sin_port;
                        iter = clients_queue.erase(iter);
                        break;
                    }
                }
                printf("Client %s:%d disconnected.", del_caddr, del_port);
                closesocket(socket);
                return 0;
            case GET_TIME:
